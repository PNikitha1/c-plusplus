...
void Battery::charge(double hours){
    if(hours < 0)
        throw std::invalid_argument("Charge hours cannot be negative" + std::to_string(hours));
...
void Battery::discharge(double hours){
    if(hours < 0)
        throw std::invalid_argument("Discharge hours cannot be negative" +  std::to_string(hours));
...
std::istream &operator>>(std::istream &input, Battery &b) 
{
        ...
        if (b.m_chargeLevel < 0) {
            throw std::invalid_argument("Battery charge level cannot be negative.");
        }
        if (b.m_voltage < 0) {
            throw std::invalid_argument("Battery voltage cannot be negative.");
        }
        if (b.m_temperature < 0) {
            throw std::invalid_argument("Battery temperature cannot be negative.");
        }
...
double& Battery::operator[](const std::string& parameter){
    if(parameter == "capacity")
        ...
        throw std::invalid_argument("Invalid parameter: " + parameter);
    }    
}